...
var minutos = 0;
var segundos = 0;
var app_path = (location.pathname.toLowerCase().search("cdmxcontigo/en/")!=-1?"../../chat/":(location.pathname.toLowerCase().search("cdmxcontigo/")!=-1?"../chat/":""));
...
function Crear_Ventana_Cliente(chatboxtitle){
    ...
    if($("#id_inst").val()!=4){
        if(app_path=="../chat/"){            
            ...
            var meesage_close = "Cerrar Conversaci&oacute;n";
            var message_escribiendo = " esta escribiendo...";
        }else{
            message = "Welcome to Locatel's online services.<br>How can I help you?";
            var meesage_close = "Close Conversation";
            var message_escribiendo = " is writting...";
        }
        opt=1;
    }else{
	...
	cadena+= '<a href="#" onclick="Cerrar_Ventana_Cliente(\''+chatboxtitle+'\')" alt="'+meesage_close+'" title="'+meesage_close+'"><img id="c_'+chatboxtitle+'" name="c_'+chatboxtitle +'" src="'+app_path+'images/Locatel_Close.png" border="0" style="margin-top:1px;" /></a></div><br clear="all"/></div>';
    ...
    cadena+= '<div id="msg_escribiendo" name="msg_escribiendo" class="msg_aviso" style="display:none;">'+chatboxtitle+message_escribiendo+'</div>';
    ...
    $.ajax({
		...
		success: function(data){
			if($("#id_conversacion").length){
                ...
                if(app_path=="../chat/"){
                    var message_close_sesion = "La sesi&oacuten fue terminada. Gracias por usar los servicios en linea.";
                    var message_close_button = "Volver a Iniciar Sesi&oacute;n";
                }else{
                    var message_close_sesion = "The session was ended. Thank you for using the services online.";
                    var message_close_button = "Access to the chat";
                }

                $(".chatboxcontent").append('<div style="color:#ff0000;" class="chatboxmessage"><p style="font-weight:bold">'+message_close_sesion+'<br><br><button class="btn btn-info" type="button" onClick="nueva_sesion()">'+message_close_button+'</button></p></span></div>');
                $("body").css("overflow","visible");
...
function nueva_sesion(){
    ...
    $("#id_inst").val(1);
    if(app_path=="../../chat/"){
        window.location = "http://locatel.cdmx.gob.mx/CDMXContigo/en/";
    }else{
        window.location = "http://locatel.cdmx.gob.mx/CDMXContigo/";
    }
}
