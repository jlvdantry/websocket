		window.Chat = function (){
			...
			if(!emailCheck($.trim($('#tmail').val()))){
				$('#dmsg').html("<font style='color:#B61B1C'><b>Email No valido!</b></font>");
				$('#dmsg').focus();
				return;
			}
...
function Crear_Ventana_Cliente(chatboxtitle){
	...
	$(" <div />" ).attr("id","chatbox_"+chatboxtitle).addClass("chatbox col-12").html(cadena).appendTo($("#vtn_chat"));
	$("#chatbox_"+chatboxtitle+" .chatboxtextarea").blur(function(){Remove_Focus(2,chatboxtitle)}).focus(function(){Remove_Focus(1,chatboxtitle)});
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(valores[0]==3){
							...
							$(".chatboxcontent").append('<div style="color:red;" class="chatboxmessage"><p style="font-weight:bold">La sesi&oacute;n fue terminada. Gracias por usar los servicios en linea.<br><br><br><button class="btn btn-info" type="button" onClick="nueva_sesion()">Volver a Iniciar sesi&oacuten</button></p></span></div>');
							$("#chatbox_"+lop+" .chatboxcontent").scrollTop($("#chatbox_"+lop+" .chatboxcontent")[0].scrollHeight);
